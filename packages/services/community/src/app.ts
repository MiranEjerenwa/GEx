...
export function createApp(options: AppOptions): express.Application {
  ...
  app.get('/community/health', (_req: Request, res: Response) => {
    res.status(200).json({ status: 'healthy', service: 'community-service' });
  });

  app.use('/community', createCommunityRouter(controller));
